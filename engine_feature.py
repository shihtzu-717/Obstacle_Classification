...
def prediction_several_models_cls_name(args, device):
    ...
    for result_k, result_v in result_dict.items():
        ...
        if args.pred_eval:
            # target class 번호의 합이 0 이상? 테스트 데이터 클래스 분류가 안되어 있으면 np.sum()이 음수임 
            if np.sum(np.array(result_v)[..., 2]) < 0: 
                ...
                # trash, obstacle, negative 타입으로 변경
                if args.use_type:
                    y_pred = [0 if i in [0, 1, 4] else i for i in y_pred]
                    y_target = [0 if i in [0, 1, 4] else i for i in y_target]
                    y_pred = [11 if i in [14] else i for i in y_pred]
                    y_target = [11 if i in [14] else i for i in y_target]
                    y_pred = [13 if i in [10, 11, 12, 14] else i for i in y_pred]
                    y_target = [13 if i in [10, 11, 12, 14] else i for i in y_target]
